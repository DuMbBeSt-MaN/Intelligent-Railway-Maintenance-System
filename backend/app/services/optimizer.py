...
def build_train_occupancy(
    ...
    for train in trains:
        for occ in train["occupancies"]:
            occupancy.setdefault(occ["track"], []).append(
                (occ["start"], occ["end"], train["id"])
            )
...
def generate_candidates(
    data: dict,
) -> Tuple[Dict[str, List[Candidate]], Dict[str, dict]]:
    ...
    # Which possessions are available on each track?
    ...
    # Which crews have the required skill for each job?
    eligible_crews: Dict[str, List[dict]] = {}

    for job in jobs:
        eligible_crews[job["id"]] = [
            c
            for c in crews
            if job["required_skill"] in c["skills"]
        ]

    # Which machines can perform each job?
    ...
    # Generate all feasible candidate placements.
    for job in jobs:
        ...
        for p in track_possessions:
            ...
            # Align start time to configured time step.
            aligned_start = (
                (min_start + step - 1) // step
            ) * step
            ...
            for start in range(
                ...
                # HARD CONSTRAINT:
                # Maintenance cannot overlap a train occupying
                # the same track.
                if any(
                    overlaps(start, end, t0, t1)
                    for t0, t1, _ in train_occ.get(track, [])
                ):
                    continue
                ...
                for crew in eligible_crews[job["id"]]:
                    ...
                    # HARD CONSTRAINT:
                    # Crew must be available for the entire job.
                    ...
                    for machine in eligible_machines[job["id"]]:
                        ...
                        # HARD CONSTRAINT:
                        # Required machine must be available.
                        if (
                            machine is not None
                            and not within_any_window(
                                start,
                                end,
                                machine["available"],
                            )
                        ...
                        # SOFT OBJECTIVE:
                        # Prefer the preferred start time.
                        if preferred is not None:
                            cost += (
                                abs(start - preferred)
                                * weights[
                                    "preferred_start_deviation_per_minute"
                                ]
                            )

                        # SOFT OBJECTIVE:
                        # Avoid moving already-approved jobs.
                        if existing is not None:
                            cost += (
                                abs(start - existing)
                                * weights[
                                    "schedule_change_per_minute"
                                ]
                            )
...
def solve(data: dict) -> dict:
    ...
    for job_id, job in job_by_id.items():
        ...
        vars_for_job = [
            x[(job_id, i)]
            for i in range(len(candidates[job_id]))
        ]
        ...
        if job["mandatory"]:
            ...
            if not vars_for_job:
                return {
                    ...
                    "metrics": {
                        "reason": (
                            f"Mandatory job {job_id} has no "
                            "feasible candidate placements "
                            "before optimisation."
                        )
                    },
                }
        ...
        # Optional job:
        # zero or one placement.
        else:
            model.Add(sum(vars_for_job) <= 1)

    # ---------------------------------------------------------
    # RESOURCE / TRACK CONFLICTS
    # ---------------------------------------------------------
    ...
    for a in range(len(flat)):
        ...
        for b in range(a + 1, len(flat)):
            ...
            same_machine = (
                c1.machine_id is not None
                and c1.machine_id == c2.machine_id
            )

            # HARD CONSTRAINT:
            # Two overlapping jobs cannot share:
            # - track
            # - crew
            # - machine
            if (
    ...
    for job_id, options in candidates.items():
        ...
        # SOFT OBJECTIVE:
        # Reward scheduling optional jobs according
        # to their priority.
    ...
    if status not in (
        ...
        return {
            ...
            "summary": {
                ...
                "mandatory_scheduled": 0,
                "optional_scheduled": 0,
                "jobs_unscheduled": len(job_by_id),
            },
    ...
    unscheduled = [
        ...
        if jid not in scheduled_ids
    ]

    optional_scheduled = sum(
        1
        for jid in scheduled_ids
        if not job_by_id[jid]["mandatory"]
    )

    mandatory_scheduled = sum(
        1
        for jid in scheduled_ids
        if job_by_id[jid]["mandatory"]
    )
    ...
    return {
        ...
        # Keep the original metrics so nothing
        # currently using the API breaks.
        "metrics": {
            "jobs_total": len(job_by_id),
            "jobs_scheduled": len(scheduled_ids),
            "mandatory_scheduled": mandatory_scheduled,
